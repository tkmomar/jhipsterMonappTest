...
type RestOf<T extends IPost | NewPost> = Omit<T, 'dateCreation'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      dateCreation: post.dateCreation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      dateCreation: restPost.dateCreation ? dayjs(restPost.dateCreation) : undefined,
    };
  }
